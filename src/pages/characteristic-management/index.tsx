...
const CharacteristicListPage = () => {
    ...
    const onDeleteAsync = async () => {
        try {
            await characteristicService.DeleteCharacteristicAdmin(
                idSelected,
                setLoading
            ).then((res) => {
                if (res) {
    ...
    const onNavigate = (id: any) => {
        router(`${(ROUTE_PATH.VIEW_CHARACTERISTIC_MANAGEMENT).replace(`${Constants.UseParams.Id}`, "")}${id}`);
    }
    ...
    return (
        ...
        >
            <div className={styles.manage_container}>
                <h2>Quản lý tính năng sản phẩm</h2>
                <div className={styles.searchBar}>
                    ...
                    />
                    <ButtonHref
                        href={ROUTE_PATH.ADD_CHARACTERISTIC_MANAGEMENT}
                        title={'Thêm mới'}
                        width={150}
                ...
                <DialogConfirmCommon
                    message={"Bạn có muốn xóa tính năng sản phẩm này ra khỏi hệ thống"}
                    titleCancel={"Bỏ qua"}
                    titleOk={"Xóa"}
